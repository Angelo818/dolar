...
-- Verifica o mundo atual baseado no PlaceId do jogo
if game.PlaceId == 2753915549 then
    World1 = true
elseif game.PlaceId == 4442272183 then
    World2 = true
...
-- Função para auto farm de XP infinito
function InfiniteXP()
    spawn(function()
        while true do
            pcall(function()
                ...
                if humanoid then
                    humanoid.Health = humanoid.MaxHealth -- Mantém o jogador vivo
                end
                
                -- Simula a conclusão de uma missão repetidamente
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CompleteQuest", "AutoXPQuest")
                
                -- Aguarda um curto período para evitar sobrecarga
                wait(0.5)
            end)
        end
    end)
...
-- Função para ganhar XP automaticamente sem fazer nada
function PassiveXP()
    spawn(function()
        while true do
            pcall(function()
                -- Simula o ganho de XP automaticamente
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AddXP", 1000) -- Ajuste o valor conforme necessário
            end)
            wait(1) -- Ganha XP a cada segundo
        end
    end)
end

-- Função para acessar todas as raids do Sea 2 e Sea 3, incluindo Dough
function UnlockAllRaids()
    spawn(function()
        pcall(function()
            for _, raid in pairs({"Flame", "Ice", "Quake", "Light", "Dark", "String", "Rumble", "Magma", "Buddha", "Sand", "Phoenix", "Dough"}) do
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("UnlockRaid", raid)
            end
        end)
    end)
end

-- Função para ficar invisível para outros jogadores
            ...
            if player.Character then
                for _, part in pairs(player.Character:GetChildren()) do
                    if part:IsA("BasePart") or part:IsA("MeshPart") then
                        part.Transparency = 1 -- Torna todas as partes do corpo invisíveis
                        part.CanCollide = false -- Evita colisões desnecessárias
                    end
                end
            end
        end)
...
-- Função para obter a Yoru V2 com um clique
function GetYoruV2()
    spawn(function()
        pcall(function()
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("UpgradeSword", "YoruV2")
        end)
    end)
end

-- Chama as funções de XP infinito, XP passivo, desbloqueio de raids, invisibilidade e Yoru V2
InfiniteXP()
PassiveXP()
UnlockAllRaids()
BecomeInvisible()
...
-- Lista de frutas do jogo, das mais comuns às mais raras
local fruitList = {
    "Spin", "Chop", "Spring", "Bomb", "Spike", "Smoke", "Flame", "Falcon", "Ice", "Sand", 
    "Dark", "Diamond", "Light", "Love", "Rubber", "Barrier", "Magma", "Quake", "Buddha", "String", 
    "Phoenix", "Rumble", "Paw", "Gravity", "Dough", "Shadow", "Venom", "Control", "Spirit", "Dragon", "Leopard"
}

-- Função para fazer chover frutas por 5 minutos
function RainFruits()
    spawn(function()
        local startTime = tick()
        while tick() - startTime < 300 do -- 300 segundos = 5 minutos
            pcall(function()
                local fruit = Instance.new("Tool")
                local randomFruit = fruitList[math.random(1, #fruitList)]
                fruit.Name = randomFruit
                fruit.Parent = game.Workspace
                fruit.Handle = Instance.new("Part", fruit)
                fruit.Handle.Size = Vector3.new(2,2,2)
                fruit.Handle.Position = game.Players.LocalPlayer.Character.Head.Position + Vector3.new(math.random(-50, 50), 50, math.random(-50, 50))
            end)
            wait(1) -- Cria uma fruta a cada segundo
        end
    end)
end

-- Chama a função de chuva de frutas
RainFruits()
